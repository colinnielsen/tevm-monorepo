...
const hexToBytes = (hex) => {
	...
	/* c8 ignore next - odd-length hex normalization rarely needed but protects against edge cases */
				...
				const shape = {
					takeSnapshot: () =>
						Effect.gen(function* () {
							...
							for (const [address, accountStorage] of Object.entries(state)) {
								...
								/* c8 ignore next - defensive check for AccountStorage type guard */
					...
					revertToSnapshot: (/** @type {Hex} */ id) =>
						Effect.gen(function* () {
							...
							// Step 3b: Defensive NaN check for corrupted snapshot IDs (Fix for #R148-P3-005)
							// This should never happen since IDs are generated by toHex(), but prevents
							// silent failures if the Map is somehow corrupted with invalid keys
							/* c8 ignore start - defensive code: IDs are always generated with valid hex */
							if (Number.isNaN(targetNum)) {
								return yield* Effect.fail(
									new SnapshotNotFoundError({
										snapshotId: id,
										message: `Invalid snapshot id format: ${id}. Expected valid hex string.`,
									}),
								)
							}
							/* c8 ignore stop */
